...
for contrast in [0.1, 0.25, 0.5, 0.75, 1.0, 2.0]:
    ...
    im = mc.rectif(mc.random_cloud(mc.envelope_color(fx, fy, ft, alpha=1.)), contrast, method='energy')
    mc.anim_save(im, os.path.join(mc.figpath, name_), display=False)

for seed in [123456 + step for step in range(7)]:
    name_ = name + '-seed-' + str(seed)
    mc.anim_save(mc.rectif(mc.random_cloud(mc.envelope_color(fx, fy, ft, alpha=1.), seed=seed)), os.path.join(mc.figpath, name_), display=False)
